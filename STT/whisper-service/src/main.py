"""
Main application entry point for Whisper STT Microservice.

Initializes:
- FastAPI application
- Whisper worker with warmup
...
from .inference import whisper_worker as ww_module
from .inference.whisper_worker import WhisperWorker
    ...
    while True:
        try:
            ...
            if iq_module.inference_queue is not None:
                ...
                stt_queue_depth.set(queue.depth)
                update_queue_metrics(
                    queue_depth=queue.depth,
                    estimated_server_total_ms=queue.estimated_server_total_ms,
                    avg_inference_ms=queue.avg_processing_ms,
                )
            
            # Update active sessions
            stt_active_sessions.set(session_manager.active_sessions)
...
async def lifespan(app: FastAPI):
    ...
    Startup:
    1. Configure logging
    2. Create and load Whisper worker
    3. Warm up model
    4. Create and start inference queue
    ...
    # Configure logging
    configure_logging()
    logger.info("Starting Whisper STT Microservice...")
    
    # 1. Create worker and load model
    logger.info(
        f"Creating Whisper worker with model: {settings.whisper_model} "
        f"(compute_type={settings.compute_type})"
    )
    worker = WhisperWorker()
    
    # Load model (this may take a while)
    logger.info("Loading Whisper model (this may take a minute)...")
    await asyncio.to_thread(worker.load_model)
    
    # Set global worker
    ww_module.whisper_worker = worker
    ...
    logger.info(
        f"Whisper STT ready - listening on {settings.host}:{settings.port}"
    )
    
    yield
    
    # Shutdown
    logger.info("Shutting down Whisper STT Microservice...")
...
app = FastAPI(
    ...
    description="Real-time STT with Faster-Whisper for Brazilian Portuguese",
    version="0.1.0",
    lifespan=lifespan,
)
