...
class ErrorCode(str, Enum):
    ...
    QUEUE_FULL = "QUEUE_FULL"  # Queue depth exceeded
    QUEUE_CONGESTION = "QUEUE_CONGESTION"  # Estimated wait too high
...
class SessionConfig(BaseModel):
    ...
    voice: Literal["F1", "F2", "F3", "F4", "F5", "M1", "M2", "M3", "M4", "M5"] = Field(
        default="F1",
        description="Voice to use for TTS (F1-F5 female, M1-M5 male)",
    )
    speed: float = Field(
        default=1.0,
        ge=0.5,
